...
<body class="bg-gray-100 p-10">
    <h1 class="text-2xl font-bold mb-4">Editar Aula (CU-005)</h1>
    ...
    <div class="bg-white p-6 rounded-xl shadow border max-w-2xl">
        <form action="{{ route('coordinador.aulas.update', $aula) }}" method="POST" class="space-y-4">
            @csrf @method('PUT') <div>
                <label for="numero" class="block text-sm font-medium text-gray-700">Número de Aula</label>
                <input type="text" id="numero" name="numero" value="{{ old('numero', $aula->numero) }}" required
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>

            <div>
                <label for="piso" class="block text-sm font-medium text-gray-700">Piso</label>
                <input type="number" id="piso" name="piso" value="{{ old('piso', $aula->piso) }}" required
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
            ...
            <div>
                ...
                <select id="activo" name="activo" 
                    ...
                    <option value="1" @selected(old('activo', $aula->activo) == 1)>Activo</option>
                    <option value="0" @selected(old('activo', $aula->activo) == 0)>Inactivo</option>
                </select>
            </div>
